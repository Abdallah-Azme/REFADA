...
interface MaritalStatusFormDialogProps {
  ...
  initialData?: MaritalStatus | null;
  onSubmit: (data: MaritalStatusFormValues) => void;
  isPending?: boolean;
}

export function MaritalStatusFormDialog({
  open,
  onOpenChange,
  initialData,
  onSubmit,
  ...
  const handleSubmit = (data: MaritalStatusFormValues) => {
    onSubmit(data);
    if (!isPending) {
      form.reset();
    }
